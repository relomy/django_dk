...
"""
    ...
    def add_arguments(self, parser):
        ...
        parser.add_argument(
            ...
            default=50,
            help=(
                "Percentile to split the data (cohort level - used in"
    ...
    def handle(self, *args, **options):
        ...
        if options["ownerships_contest"]:
            for contest_id in options["ownerships_contest"]:
                ...
                print("==================")
                print(f"{contest.dk_id}, {contest.date}, {contest.name}")
        ...
        if options["lineup"]:
            if options["weighted_scores"]:
                ...
                lineup_scripts.generate(score_scripts.median_points_per_salary())
        if options["validate"]:
            if options["weighted_scores"]:
                validation_scripts.run(
                    result_scripts.get_weighted_scores, lineup_scripts.generate
                )
            elif options["score_points_per_salary"]:
                validation_scripts.run(
                    score_scripts.median_points_per_salary, lineup_scripts.generate
                )
            elif options["score_linear_regression"]:
                validation_scripts.run(
                    score_scripts.linear_regression, lineup_scripts.generate
                )
